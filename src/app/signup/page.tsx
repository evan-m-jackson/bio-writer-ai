...
export default function Signup () {
    const initialInputs = {firstName: "", lastName: "", email: "", password: "", confirmPassword: ""};
    const [inputs, setInputs] = useState(initialInputs);
    const [error, setError] = useState("");

    const handleChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        setInputs((prevInputs) => ({...prevInputs, [event.target.id]: event.target.value}));
    }
    ...
    const handleSubmit = (event: React.ChangeEvent<HTMLFormElement>) => {
        ...
        if (hasBlankInput){
            setError("Error: All fields must be populated.");
        } else if (inputs.password != inputs.confirmPassword) {
            setError("Error: Passwords must match.")
        } else if (inputs.password.length < 8 || inputs.password.length > 16) {
            setError("Error: Password must be between 8-16 characters.")
        } else {
            setError("");
            axios.post('/api', inputs).then((response) => {console.log(response)}).catch((e) => {console.log(e)})
    ...
    return (
        <div className="w-full max-w-md text-black dark:text-white">
            ...
            <div className="relative z-0 my-3">
                <input type="text" id="firstName" className="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " value={inputs.firstName} onChange={handleChange} />
            ...
            <div className="relative z-0 my-3">
                <input type="text" id="lastName" className="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " value={inputs.lastName} onChange={handleChange} />
            ...
            <div className="relative z-0 my-3">
                <input type="email" id="email" className="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " value={inputs.email} onChange={handleChange} />
            ...
            <div className="relative z-0 my-3">
                <input type="password" id="password" className="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " value={inputs.password} onChange={handleChange} />
            ...
            <div className="relative z-0 my-3">
                <input type="password" id="confirmPassword" className="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " value={inputs.confirmPassword} onChange={handleChange} />
            ...
            <button type="submit" className="text-white bg-purple-700 hover:bg-purple-800 focus:outline-none focus:ring-4 focus:ring-purple-300 font-medium rounded-full text-sm px-5 py-2.5 text-center mb-2 dark:bg-purple-600 dark:hover:bg-purple-700 dark:focus:ring-purple-900">Create</button>
            </div>
            </form>
